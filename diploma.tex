...
{
  name=čistopis,
  description={(angl.\ plaintext)},
}

\newglossaryentry{tajnopis}
{
  name=tajnopis,
  description={(angl .ciphertext)},
}

\newglossaryentry{enckey}
{
  name=šifrirni ključ,
  description={(angl. encryption key)},
}
...
\newglossaryentry{neizpodbitnost}
{
  ...
  description={(angl.\ non-repudiation)},
}

\newglossaryentry{zaupnost}
{
  name=zaupnost,
  description={(angl.\ confidentiality)},
}

\newglossaryentry{celovitost}
{
  name=celovitost,
  description={(angl.\ integrity)},
}

\newglossaryentry{availability}
{
  name=razpoložljivost,
  description={(angl. availability)},
}

\newglossaryentry{overjanje}
{
  name=overjanje,
  description={(angl.\ authentication)},
}

\newglossaryentry{avtorizacija}
{
  name=avtorizacija,
  description={(angl.\ authorization)},
}
...
\newglossaryentry{cold-boot-attack}
{
  ...
  description={(angl. cold boot attack)},
}
...
The goal of this diploma thesis is to present an overview of data security in cloud systems. The first part covers basics of cloud and security. Additionaly it identifies some risks arising from use of cloud computing. The second part is focused on cryptography, especially on ciphers, as a tool for mitigation of those risks. We try to convey some recent ideas from the cryptographic world, that are especially interesting in the context of cloud computing, to the reader in an easy manner. At the end, we give some tips on how to ensure security of data.
...
Varnost je širok pojem, ki ga v kontekstu računalništva lahko razdelimo na tri dele. Računalniško varnost, informacijsko varnost in varovanje podatkov.
...
\begin{description}
  ...
  \item[Varovanje podatkov (angl. information assurance)] se ukvarja z varovanjem podatkov in upravljanjem tveganj povezanih z uporabo, prenosom in procesiranjem podatkov s ciljem zagotavljanja integritete, dostopnosti, zaupnosti in neizpodbitnosti podatkov.
\end{description}
...
Ob prelomu tisočletja se je izkazalo, da je \gls{DES}-ov 64-bitni ključ prešibak, da bi zadržal napade s surovo silo. Zato se je pojavila varianta algoritma imenovana 3DES.\ 3DES uporablja 3 ključe, pri čemer imamo na voljo 3 opcije za izbiro ključev.
