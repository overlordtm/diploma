...
Napadalcu sta že znana oba tajnopisa $c_{i-1}$ in $c_{j-1}$, zato če ugane bodisi vrednost $m_j$, bodisi vrednost $m_i$, lahko preko zgornje formule izračuna manjkajočo vrednost in tako dešifrira delček sporočila.
\end{mdframed}
...
Protokol \gls{TLS} uporablja simetrične kriptosisteme za zagotavljanje zasebnosti, kriptografijo javnih ključev za ugotavljanje istovetnosti strežnika s katerim komuniciramo in za dogovor o ključu ter kode za overjanje, s katerimi zagotavljanje celovitosti podatkov. Točne detajle implementacije je možno najti v literaturi, kot je knjiga \textit{Cryptography and network security} avtorja W. Stallings \cite{stallings2006cryptography}. Ker protokol ne predpisuje točno določenih algoritmov, ampak prepušča izbor le teh uporabniku, je pomembno, da namenimo nekaj časa pravilni nastavitvi protokola.
...
\begin{description}[style=nextline]
  \item[DROWN] Napad napadalcu omogoča dešifriranje celotne komunikacije, zaščitene s (sicer pravilno nastavljen) TLS. Za uspešen napad je potreben strežnik, ki uporablja zastarel protokol SSL v2 in isti zasebni ključ kot žrtvin strežnik~\cite{aviramdrown}.
  \item[BEAST]
