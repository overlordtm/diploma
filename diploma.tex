...
\todo{rename v asimetricni kriptosistemi}
...
\todo{Izmenjava -> dogovor o kljucu}
...
Izkaže se, da take šifrirne sheme res obstajajo, vendar je to področje kriptografije še dokaj novo in obstaja še veliko odprtih problemov, vendar prvi rezultati so obetavni. Čeprav so bili mnogi algoritmi, ki jih sedaj uvrščamo pod funkcionalne šifrirne sheme odkriti že prej, pa je bila prva formalizacija javno predstavljena šele leta 2011~\cite{boneh2011functional}.

V takih šifrirnih shemah posebni dešifrirni ključ omogoča uporabniku izračun vrednosti neke funkcije nad šifriranimi podatki. V funkcionalni šifrirni shemi za funkcionalnost~$F(\cdot, \cdot)$ lahko \gls{authority}, ki si lasti glavni skrivni ključ, ustvari ključ~$sk_k$, s katerim lahko uporabnik izračuna vrednost funkcije~$F(k, x)$ iz tajnopisa za~$x$.

\begin{mdframed}[frametitle={Funkcionalnost F}]
...
\subsubsection{Šifriranje na osnovi predikatov}
\label{subs:Šifriranje na osnovi predikatov}

Šifriranje na osnovi predikatov je eden najnovejših pristopov na področju kriptografije javnih ključev. V tradicionalni kriptografski shemi javnih ključev se sporočilo šifrira za točno določenega prejemnika (tistega, kateremu pripada javni ključ, ki ga uporabimo za šifriranje). Korak dlje je šlo šifriranje na osnovi identitete (angl. \acrlong{IBE}), kjer lahko za javni ključ uporabimo kar preprost niz znakov, kot je recimo prejemnikov e-naslov. Šifriranje na osnovi predikatov pa je šlo še korak dlje, saj dovoljuje, da isti tajnopis dešifrira kdorkoli poseduje ključ, ki zadovolji vse predikate.

Poglejmo si za primer Anito, ki shrani šifrirane podatke v oblak. Anita ima datoteko $X$, ki bi jo rada delila s skupinama \textbf{Prijatelji} in \textbf{Znanci}. Prav tako ima datoteko $Y$, ki bi jo rada delila s skupinama \textbf{Družina} in \textbf{Prijatelji}. To lahko stori tako, da ustvari en ključ za datoteko $X$ in ta ključ deli s \textbf{Prijatelji} in  \textbf{Znanci}, nato ustvari drug ključ za datoteko $Y$ in ta ključ deli s skupinama \textbf{Družina} in \textbf{Prijatelji}. Problem je, da mora Anita za vsako datoteko, ki jo želi deliti, ustvariti nov ključ.
